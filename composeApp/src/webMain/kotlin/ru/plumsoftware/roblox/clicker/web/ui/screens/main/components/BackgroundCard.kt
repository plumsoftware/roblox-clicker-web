...
fun BackgroundCard(
    ...
    val backgroundColor = when {
        background.isSelected -> Color(0xFF00E676).copy(alpha = 0.5f) // Зеленый (Выбран)
        background.isUnlocked -> Color.White.copy(alpha = 0.2f)       // Обычный (Куплен)
        else -> Color.Black.copy(alpha = 0.3f)                       // Темный (Закрыт)
    }

    val borderColor = if (background.isSelected) Color(0xFF00E676) else Color.Transparent
    ...
    Card(
        ...
        Column(
            ...
            Row(
                ...
                Image(
                    painter = painterResource(background.resourceName),
